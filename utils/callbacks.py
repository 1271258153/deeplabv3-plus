...
class TeeLogger:
    ...
    def start(self):
        ...
        # 拦截stdout.write，累积tqdm行，遇到\n写入log
        def custom_write(message):
            original_write(message)
            if '\r' in message:
                self._current_line = ''
            elif '\n' in message:
                clean = re.sub(r'\033\[[0-9;]*m', '', self._current_line).strip()
...
class LossHistory():
    ...
    def start_logging(self):
        """将print输出重定向到 .log 文件，tqdm保持原始终端"""
        log_path = os.path.join(self.log_dir, "train.log")
        self.tee = TeeLogger(log_path)
        self.tee.start()
    ...
    def append_loss(self, epoch, loss, val_loss, lr=None, f_score=None):
