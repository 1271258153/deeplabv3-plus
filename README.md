...
4. 如果不是jpg图片，执行`python png2jpg.py`将png图片转换成jpg图片，默认覆盖`VOCdevkit/VOC2007/JPEGImages`路径。
5. 在训练前利用voc_annotation.py文件生成对应的txt，让ai分析是否要开启`train.py`的`dice_loss`和`focal_loss`。    
6. 在train.py文件夹下面，选择自己要使用的主干模型和下采样因子。本文提供的主干模型有mobilenet和xception。下采样因子可以在8和16中选择。需要注意的是，预训练模型需要和主干模型相对应。   
7. 注意修改train.py的num_classes为分类个数+1，需要修改很多文件，让ai检查一下。    
8. 运行`python train.py && /usr/bin/shutdown`即可开始训练，做对比实验需要和其它的模型的训练参数一致。

    | 参数 | DeepLabV3+ (调整后) | PIDNet Small | 状态 |
    |------|---------------------|-------------|------|
    | num_classes | 10 | 10 | ✅ 一致 |
    | 优化器 | SGD | SGD | ✅ 一致 |
    | 初始学习率（Init_lr） | 0.0133×(12/16)=0.009975≈0.01 | 0.01 | ✅ 已对齐 |
    | weight_decay | 5e-4 | 5e-4 | ✅ 已对齐 |
    | momentum | 0.9 | 0.9 | ✅ 一致 |
    | Nesterov | True | False | ⚠️ 不同 |
    | 学习率调度 | cosine | polynomial (power=0.9) | ⚠️ 最接近的选项 |
    | 总 Epoch | 200 (冻结50+解冻150) | 200 | ✅ 已对齐 |
    | Batch Size | 12→12 | 12 | ✅ 已对齐 |
    | 损失函数 | CE + Dice + Focal | OHEM + Boundary | ⚠️ 架构差异 |
    | 输入尺寸（input_shape） | 512×512 | 640×640 | ⚠️ DeepLab默认 |
    | 数据增强 | 标准(resize+翻转) | multi-scale + flip | ⚠️ PIDNet更强 |
    | 保存/评估间隔 | 10 epoch | best.pt | ✅ |
